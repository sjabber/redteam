...
(
    ...
    smtp_host     text      default 'smtp.naver.com'::text,
...
(
    p_no          serial              not null
        constraint project_info_pk
            primary key,
    tml_no        integer   default 0 not null
    ...
    p_status      smallint  default 0 not null,
    un_send_no    integer   default 0 not null
);
...
(
    ...
    project_no        integer not null
        constraint count_info_project_info_p_no_fk_2
...
create table project_target_info
(
    p_no      integer           not null
        constraint project_detail_info_project_info_p_no_fk
            references project_info
            on delete cascade,
    tml_no    integer default 0 not null
        constraint project_detail_info_template_info_tmp_no_fk
            references template_info
            on delete set default,
    target_no integer,
    user_no   integer
        constraint project_detail_info_user_info_user_no_fk
            references user_info
            on delete cascade
);

alter table project_target_info
    owner to postgres;
